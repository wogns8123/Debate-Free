...
import { DiscussionStatusDto, VoteResultsDto, Debate } from '../types';
...
export const DiscussionControls: React.FC<DiscussionControlsProps> = ({
  ...
  const forPercentage = totalVotes > 0 ? ((voteResults!.for / totalVotes) * 100).toFixed(1) : '0.0';
  const againstPercentage = totalVotes > 0 ? ((voteResults!.against / totalVotes) * 100).toFixed(1) : '0.0';
  ...
  return (
    <div className="w-full mx-auto p-4 bg-gray-800 text-white rounded-lg shadow-lg mb-4">
      ...
      <div className="flex justify-center space-x-4 mb-4">
        ...
        {currentStatus === 'debating' && ( 
          <Button onClick={onStartVoting} className="bg-purple-600 hover:bg-purple-700">
            투표 시작
          </Button>
        )}
        {(currentStatus === 'debating' || currentStatus === 'voting' || currentStatus === 'paused') && ( 
